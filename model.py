        ...
        for device in model.K:
            ...
            for config in model.C:
                model.loss_constraint.add(model.x[gateway, position, device, config] * 
                                          (configurations[config].tp - referencePrx - 
                                           10 * attenuationExponent * math.log10(
                                               math.sqrt((gw_p[0] - ed_p[0])**2 + 
                                                          (gw_p[1] - ed_p[1])**2 +
                                                          (gw_p[2] - ed_p[2])**2)/referenceDistance)
...
for key in model.x:
    if model.x[key].value > 10**(-6):
        print('drone={}, {}, device={}, {}'.format(key[0], gateway_positions[key[1]], key[2], configurations[key[3]]),
              ' -> ', model.x[key].value)
        if (gateway_positions[key[1]].x, gateway_positions[key[1]].y, gateway_positions[key[1]].z) not in uniqueGatewayPositions:
            gatewaysPositions += str(gateway_positions[key[1]].x) + " " + str(gateway_positions[key[1]].y) + " " + str(
                gateway_positions[key[1]].z) + "\n"
            uniqueGatewayPositions.append((gateway_positions[key[1]].x, gateway_positions[key[1]].y, gateway_positions[key[1]].z))
